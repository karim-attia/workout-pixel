...
class OtherActions @Inject constructor(
	...
	suspend fun oneTimeSetup() {
		...
		updateAllWidgets()
		Log.d(TAG, "goalsWithoutGlanceId ${goalsWithoutGlanceId()}")
		Log.d(TAG, "goalsWithGlanceId ${goalsWithGlanceId()}")
	...
	val appWidgetIds: IntArray =
		AppWidgetManager.getInstance(context)
			.getAppWidgetIds(ComponentName(context.packageName, GlanceWidget::class.java.name))
		...
		return goals.stream()
			.filter { (_, appWidgetId) ->
				Arrays.stream(appWidgetIds)
					.noneMatch { i: Int -> appWidgetId != AppWidgetManager.INVALID_APPWIDGET_ID && i == appWidgetId }
			}
			.collect(Collectors.toList())
	}
	...
	suspend fun glanceIds(): List<GlanceId> =
		GlanceAppWidgetManager(context).getGlanceIds(GlanceWidget::class.java)
