...
class OnboardingViewModel(
	...
	private var receivedLambdas: Lambdas
) : ChatViewModel() {
	...
	private fun proposalThumbsUp(): ChatMessage = ChatMessage(
		text = "\uD83D\uDC4D",
		isMessageByUser = true,
		action = checkIfPinWasSuccessfulAndSendAccordingMessage(
			showPinDialog = true,
			successMessage = ::success,
			noSuccessMessage = ::waitingForCallback
		)
	)
	...
	*/
	...
	private fun checkIfPinWasSuccessfulAndSendAccordingMessage(
		...
		viewModelScope.launch {
			...
			if (timeout != null) {
				delay(timeout)
				if (!success) noSuccessMessage?.let {
					insertMessageBuilderToQueueAtNextPositionAndAdvance(
						it
					)
				}
