...
//@Dao
public interface OldGoalDao {
    // Past Workouts
    // Return all workouts by appWidgetId
    //@Query("SELECT * FROM pastWorkouts WHERE widgetUid=:goalUid ORDER BY workoutTime DESC")
    LiveData<List<OldPastWorkout>> loadAllPastWorkouts(int goalUid);

    //@Query("SELECT * FROM pastWorkouts WHERE widgetUid=:goalUid ORDER BY workoutTime DESC")
    Flow<List<OldPastWorkout>> loadAllPastWorkoutsFlow(int goalUid);
    ...
    void insertPastWorkout(OldPastWorkout pastWorkout);

    //@Update(onConflict = OnConflictStrategy.REPLACE, entity = PastWorkout.class)
    void updatePastWorkout(OldPastWorkout pastWorkout);
